...
export default function SkillsPage() {
  ...
  const handleMultiSelectChange = (field: keyof SkillsFormData, value: string) => {
    setFormData((prev) => {
  ...
  const handleRemoveItem = (field: keyof SkillsFormData, value: string) => {
    setFormData((prev) => ({
      ...prev,
      ...
      <div className="p-10">
        ...
        <div className="p-6">
          ...
          <div className="space-y-6 p-6 mb-5 bg-[#F7F9F9] rounded-lg">
                  ...
                  {Array.isArray(formData.certifications) && formData.certifications.length > 0 && (
                    <ul className="text-xs text-center w-full max-w-sm mx-auto mt-1 space-y-1">
                      {formData.certifications.map((file, index) => (
                        <li key={index} className="text-xs text-green-700 font-medium mb-3">
                          <span className="truncate">
                            <CheckCircle className="inline w-4 h-4 mr-1" /> {file.name}
                          </span>
                          <button
                            type="button"
                            onClick={() =>
            ...
            <Footer stepNumber={3} formData={formData} />
          </div>
        </div>
      </div>
    </div>
